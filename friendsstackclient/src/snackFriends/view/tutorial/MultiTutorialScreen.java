...
/**
 * 튜토리얼 팝업 창에서 멀티플레이 버튼을 눌렀을 시 플레이어에게 제공하는 화면이다.
 * 
 * @author SnackTimeFriends
 *
 */
public class MultiTutorialScreen extends JPanel {
	private static final long serialVersionUID = 1L;
	/**
	 * 멀티플레이 튜토리얼의 모든 페이지에 대한 이미지들을 저장하고 있는 변수
	 */
	...
	private class PreviousButtonHandler extends MouseAdapter {
		 ...
		 * 
		 * @param e
		 *            MouseEvent 마우스 커서가 이전 페이지 버튼 내에 들어왔을 시 발생한다.
		 */
		 ...
		 * 
		 * @param e
		 *            MouseEvent 마우스 커서가 이전 페이지 버튼을 빠져나갔을 시 발생한다.
		 */
		 ...
		 * 
		 * @param e
		 *            MouseEvent 이전 페이지 버튼을 눌렀을 시 발생하고 현재 페이지에서 하나 전 페이지로 이동한다. 만약 이전
		 *            페이지 버튼을 눌렀을 시 현재 페이지가 첫 번째 페이지였다면, 튜토리얼의 맨 뒤 페이지로 이동한다.
		 */
		public void mousePressed(MouseEvent e) {
			...
			if (page == 0)
				...
				page--;
			pageLabel.setText((page + 1) + "/" + multi.length);
			repaint();
		}
	}
	...
	private class PauseButtonHandler extends MouseAdapter {
		...
		public void mouseEntered(MouseEvent e) {
			...
			pauseButton.setBounds(675, 892, 120, 65);
		...
		public void mouseExited(MouseEvent e) {
			...
			pauseButton.setBounds(680, 891, 110, 63);
	...
	/**
	 * 멀티플레이 튜토리얼 각각의 페이지에 해당하는 이미지와 이전페이지 버튼, 다음페이지 버튼, 일시정지 버튼에 대한 이미지를 생성하고 각각의
	 * 버튼과 페이지 JLabel을 패널에 add한다.
	 */
	public MultiTutorialScreen() {
		setLayout(null);
		multi = new ImageIcon[] { new ImageIcon("image/tutorial/multi/multiBackground0.png"),
				new ImageIcon("image/tutorial/multi/multiBackground1.png"),
				new ImageIcon("image/tutorial/multi/multiBackground2.png") };
	...
	/**
	 * return 타입: void 멀티튜토리얼 화면에 페이지 라벨을 추가하는 메서드이다.
	 */
	...
	/**
	 * paintComponent()를 통해 멀티플레이 튜토리얼 화면(MultiTutorialScreen)의 배경이미지를 그린다.
	 */
	public void paintComponent(Graphics g) {
		g.drawImage(multi[page].getImage(), 0, 0, 1440, 960, null);
	}
